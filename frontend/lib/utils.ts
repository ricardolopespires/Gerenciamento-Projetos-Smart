...
type Period = {
  from: string | Date | undefined; // Data de início no formato 'YYYY-MM-DD' ou objeto Date
  to: string | Date | undefined;   // Data de fim no formato 'YYYY-MM-DD' ou objeto Date
};
export function formatPeriod(period: Period): string {
  ...
  const fromDate = typeof period.from === 'string' ? new Date(period.from) : period.from;
  const toDate = typeof period.to === 'string' ? new Date(period.to) : period.to;

  const fromFormatted = fromDate.toLocaleDateString('pt-BR', options);
  const toFormatted = toDate.toLocaleDateString('pt-BR', options);
