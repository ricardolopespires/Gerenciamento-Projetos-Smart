...
import { recommendations, recommendationsCreated } from "./actions.recommendations";
...
export const getAssetCryptosList  = async({UserId})=>{

  try{
      const user = await getUserInfo({userId:UserId});
      const res = await AxiosInstance.get(`/api/v1/assets/crypto/${user[0].id}/list/`)      
      ...
      if(res.status == 200){
        ...
        return parseStringify({"status":404, "message":"erro ao recet os dados"})
      }



  }catch{
    return parseStringify({"status":500, "message":"error no servidor"}) 
  }
}