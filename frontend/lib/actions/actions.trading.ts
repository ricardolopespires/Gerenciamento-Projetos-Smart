...
export const getAssetList = async({CodeAsset}:getAssetListProps)=>{
    const email = "unicodireito@gmail.com"
    ...
    try{
        if (CodeAsset === "crypto"){
            ...
            const res = await getAssetsCommodities({UserId:email})
            
            return res
        }else if(CodeAsset === "currency"){
            const res = await getAssetsCurrencies({UserId:email})
            return res
        }else if(CodeAsset === "stocks"){
            const res = await getAssetsStocks({UserId:email});
            console.log(res);
            return res
        };
...
export const getListpositions = async()=>{

    try{           
        const response = await AxiosInstance.get(`/api/v1/metatrader/posicoes/`)        
        if(response.status === 200){
            return response.data
        }else{
            parseStringify({"status":400, "message":"Enviada com error"})
